...
//	var csskeyWord = ['height','width']
	...
	_H.init = function(){
		...
		this.exactWidth = DEVICEW > DEVICEH ? DEVICEH * window.devicePixelRatio : DEVICEW * window.devicePixelRatio;
	};
	_H.init();
		...
		textSubfix : function(charNum, subfix){
			...
			var processContent = function(ele,charNum, subfix){
				...
				if(!ele.length){
					...
					subfix = charNum >= text.length ? "" : subfix;
					charNum = charNum >= text.length ? text.length : charNum;
